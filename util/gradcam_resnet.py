...
class ModelOutputs:
    ...
    def __call__(self, x):
        ...
        for module in self.model.backbone.blocks:
...
class GuidedBackpropReLU(Function):
    ...
    def backward(self, grad_output):
        ...
        grad_input = torch.addcmul(
            ...
            torch.addcmul(
                torch.zeros(input_img.size()).type_as(input_img), grad_output, positive_mask_1
            ),
            positive_mask_2,
        )
        return grad_input
...
def get_args():
    ...
    parser.add_argument(
        "--use-cuda", action="store_true", default=False, help="Use NVIDIA GPU acceleration"
    )
    parser.add_argument(
