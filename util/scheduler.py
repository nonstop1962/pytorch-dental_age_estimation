...
def get_scheduler(scheduler_dict, optimizer):
    if scheduler_dict is None:
        logger.info("[OPTIMIZER] [scheduler] No LR Scheduling")
        return ConstantLR(optimizer)
    # return 'hi'
    s_type = scheduler_dict["name"]
    scheduler_dict.pop("name")

    logger.info(f"[OPTIMIZER] [scheduler] {s_type} [params] {scheduler_dict}")
    ...
    if "warmup_iters" in scheduler_dict:
        ...
        logger.info(
            f'[WARMUP] with {warmup_dict["warmup_iters"]} iters {warmup_dict["gamma"]} gamma and {warmup_dict["mode"]} mode'
        )
